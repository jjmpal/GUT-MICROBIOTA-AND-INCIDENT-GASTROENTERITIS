...
```{r, echo = FALSE}
{ dplyr::bind_rows(df_cox_results_east %>% dplyr::filter(qval < 0.05) %>% dplyr::mutate(model = "Discovery", EAST = "East"),
    ...
    ggplot2::geom_point(size = 2, stroke = 1.5, position = ggplot2::position_dodge(width = 0.6)) +
    ...
    ggplot2::geom_text(ggplot2::aes(label = prevalence, y = 3.3),
                       position = ggplot2::position_dodge(width = 0.6)) +
    ggplot2::scale_x_discrete(name = NULL, expand = c(0.05, 0.05)) +
  ...
  myggsave("gastroenteritis_cox", width = 10, height = 6)
```
