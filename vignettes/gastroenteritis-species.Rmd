...
# Matching sampleID from functional data to metadata
  ...
  tse_subset_coldata <- as.data.frame(colData(tse))
  
  #rownames to column called "Barcode"
  tse_subset_coldata <- tse_subset_coldata %>% rownames_to_column("Barcode")
...
filter_sp <- joined_pathabundance %>% 
 ...
 #grep all pathways that are associated with species 
  filter(grepl('.s__', Pathway)) %>% 
  filter(!grepl('superpathway|plants|fungi|yeast|eukaryotes|archaea', Pathway))
...
filter_sp.10 <- filter_sp.t %>% 
  ...
  #filter for pathways that are prevalent in 1%
  dplyr::filter(prevalence > 0.01)  
...
## Perform Cox's models for pathway analysis (dicho)
...
# Store pathway names (when using species specific)
pathways <- colnames(df_dicho)[grepl('.s__', colnames(df_dicho))]
#pathways <- filter_sp_pr %>% select(-Barcode) %>% colnames()

df_cox_results_pathways_dicho <- tibble::tibble(pathways = pathways) %>%
  dplyr::mutate(results = purrr::map(pathways, ~cox_model_for_taxon(df_dicho, .x, coxph_partial), .progress = TRUE)) %>%
  dplyr::filter(!is.null(results)) %>%
  dplyr::mutate(tidy = purrr::map(results, ~broom::tidy(.x, conf.int = TRUE, exponentiate = TRUE))) %>% 
  tidyr::unnest(tidy) %>%
  dplyr::filter(stringr::str_detect(term, "term")) %>% 
  dplyr::mutate(qval = p.adjust(p.value, method = "BH")) %>% 
  dplyr::arrange(qval)

write_xlsx(df_cox_results_pathways_dicho, "pathways_dich_gastro_species_str.xlsx")
```


## Perform Cox's models for pathway analysis (inverse rank)
...
df_cox_results_pathways_inv <- tibble::tibble(pathways = pathways) %>%
  dplyr::mutate(results = purrr::map(pathways, ~cox_model_for_taxon(df_inv_rank, .x, coxph_partial), .progress = TRUE)) %>%
  dplyr::filter(!is.null(results)) %>%
  dplyr::mutate(tidy = purrr::map(results, ~broom::tidy(.x, conf.int = TRUE, exponentiate = TRUE))) %>% 
  tidyr::unnest(tidy) %>%
  dplyr::filter(stringr::str_detect(term, "term")) %>% 
...
write_xlsx(df_cox_results_pathways_inv, "pathways_inv_gastro_species_str.xlsx")


```


# Visualize the functional analysis results 
```{r}
...
pathways_data <- readxl::read_excel("/home/diabhass/Documents/pathways_dich_gastro_species_str.xlsx")
sigfunc <- pathways_data %>% filter (qval < 0.05)
...
e <-sigfunc %>%
  ...
  mutate(count = n()) %>%
  ungroup() %>%
  mutate(pathway2 = ifelse(count < 2, "Others", Function)) %>%
  select(-count)%>% 
  arrange(pathway2, pathways) %>%
  mutate(pathways = factor(pathways, levels = unique(pathways)))

##Top 20 functional pathway dotplot
et <- e %>% 
  arrange(qval) %>%
  slice_head(n=20) %>%
  select(pathways, pathway2, estimate, qval)

et$pathways <- sub("\\|g__[^.]+\\.s__", "_", et$pathways)
et$pathways <- factor(et$pathways, levels = et$pathways[order(et$qval)])


funcolor<- c("Amino acid biosynthesis" = "#D53E4F", "Cofactor/Carrier/Vitamin biosynthesis" = "#F46D43", 
             "Fatty acid and lipid biosynthesis" = "#FDAE61", "Fermentation to short-chain fatty acids" = "#E6F598",
             "Others" = "#ABDDA4", "Purine nucleotide biosynthesis" = "#66C2A5", 
             "Sugar nucleotide biosynthesis" = "#3288BD")

# Create the dot plot
pathways_plot <- ggplot(et, aes(x = estimate, y = pathways, color = pathway2, size = qval)) +
  geom_point() +
...
ggsave("pathways_plot_gastro.png", plot = pathways_plot, width = 20, height = 8, dpi = 300)
